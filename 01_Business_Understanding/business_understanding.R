...
#1A. View the Business As Machine ----
...
) 
{
  ...
  productivity_costs <- net_revenue_per_employee / workdays_per_year *
    (workdays_position_open + workdays_onboarding * onboarding_efficency)
  
  #Savings of Salary & Benefits (Cost Reduction)
  salary_benefit_reduction <- salary /workdays_per_year * workdays_position_open
  
  #Estimated Turnover Per Employee
  cost_per_employee <- direct_cost + productivity_costs - salary_benefit_reduction
...
plot_attrition <- function(data, ..., .value,
  ...
  data_manipulated <- data %>%
   ...
   if (fct_rev){
     data_manipulated <- data_manipualted %>%
       mutate(name = forcats::fct_reorder(name, !!value_expr) %>% forcats::fct_rev()) %>%
       arrange(name)
...
dep_job_role_tbl %>%
  ...
  labs(
    ...
    subtitle = "Looks like Sales Executive and Laboratory Technician are biggest drivers of cost",
    x        = "Cost of Attrition"
  )
